        ...
        for i in range(T):
            ...
            for j in range(num_cars):
                ...
                if this_route["source"] > n*n:
                    ...
                    this_route["edges"].append(
                        this_route["source"] * 1000 + this_route["curloc"])
...
xsi:noNamespaceSchemaLocation="http://sumo.dlr.de/xsd/nodes_file.xsd">''', file=nodes)

        for i in range(n):
            for j in range(n):
                print('    <node id="%i" x="%d" y="%d"  type="traffic_light"/>' %
                      ((n*i + j + 1), (i+1)*500, (j+1)*500), file=nodes)
        for i in range(n):
            print('    <node id="%i" x="%d" y="%d"  type="priority"/>' %
                  ((n*n + i + 1), (i+1)*500, 0), file=nodes)
        for i in range(n):
            print('    <node id="%i" x="%d" y="%d"  type="priority"/>' %
                  ((n*(n+1) + i + 1), (n+1)*500, (i+1)*500), file=nodes)
        for i in range(n):
            print('    <node id="%i" x="%d" y="%d"  type="priority"/>' %
                  ((n*(n+2) + i + 1), (n-i)*500, (n+1)*500), file=nodes)
        for i in range(n):
            print('    <node id="%i" x="%d" y="%d"  type="priority"/>' %
                  ((n*(n+3) + i + 1), 0, (n-i)*500), file=nodes)
...
xsi:noNamespaceSchemaLocation="http://sumo.dlr.de/xsd/edges_file.xsd">''', file=edges)
        ...
        for i in range(1, (n+1)):
            print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                  ((1000*(n*n + i) + i), (n*n + i), i, get_rand_num(min_s, max_s)), file=edges)
            print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                  ((1000*i + n*n + i), i, (n*n + i), get_rand_num(min_s, max_s)), file=edges)
            all_edge_ids.append((1000*(n*n + i) + i))
            all_edge_ids.append((1000*i + n*n + i))
        for i in range(1, (n+1)):
            print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                  ((1000*(n*(n+1) + i) + (n*i)), (n*(n+1) + i), (n*i), get_rand_num(min_s, max_s)), file=edges)
            print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                  ((1000*n*i + (n*(n+1) + i)), (n*i), (n*(n+1) + i), get_rand_num(min_s, max_s)), file=edges)
            all_edge_ids.append((1000*(n*(n+1) + i) + (n*i)))
            all_edge_ids.append((1000*n*i + (n*(n+1) + i)))
        for i in range(1, (n+1)):
            print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                  ((1000*(n*(n+2) + i) + (n*n+1-i)), (n*(n+2) + i), (n*n+1-i), get_rand_num(min_s, max_s)), file=edges)
            print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                  ((1000*(n*n+1-i) + (n*(n+2) + i)), (n*n+1-i), (n*(n+2) + i), get_rand_num(min_s, max_s)), file=edges)
            all_edge_ids.append((1000*(n*(n+2) + i) + (n*n+1-i)))
            all_edge_ids.append((1000*(n*n+1-i) + (n*(n+2) + i)))
        for i in range(1, (n+1)):
            print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                  ((1000*(n*(n+3) + i) + (n*(n-i)+1)), (n*(n+3) + i), (n*(n-i)+1), get_rand_num(min_s, max_s)), file=edges)
            print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                  ((1000*(n*(n-i)+1) + (n*(n+3) + i)), (n*(n-i)+1), (n*(n+3) + i), get_rand_num(min_s, max_s)), file=edges)
        ...
        for i in range(n):
            for j in range(n-1):
                print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                      (1000*(n*i + 1 + j)+(n*i + 2 + j), (n*i + 1 + j), (n*i + 2 + j), get_rand_num(min_s, max_s)), file=edges)
                print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' %
                      (1000*(n*i + 2 + j)+(n*i + 1 + j), (n*i + 2 + j), (n*i + 1 + j), get_rand_num(min_s, max_s)), file=edges)
        ...
        for i in range(n):
            for j in range(n-1):
                print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' % (1000*(n*j +
                      1 + i)+(n*(j+1) + 1 + i), (n*j + 1 + i), (n*(j+1) + 1 + i), get_rand_num(min_s, max_s)), file=edges)
                print('    <edge id="%i" from="%i" to="%i" priority="80" numLanes="1" speed="%d" />' % (1000*(n*(j+1) +
                      1 + i)+(n*j + 1 + i), (n*(j+1) + 1 + i), (n*j + 1 + i), get_rand_num(min_s, max_s)), file=edges)
...
def run_sumo(n, summaryFile='summary.xml', tl_data=None, thetype=None):
    ...
    for j in range(nsims):
        ...
        for child in root:
            totalTravel += float(child.attrib['meanTravelTime']) * (
                float(child.attrib['ended']) - prev_num_cars_done)
...
def main(arguments):
    ...
    parser.add_argument(
        'n', help="size of grid (along one side)", type=int, default=5)
    parser.add_argument('seed', help="seed value", type=int, default=42)
    parser.add_argument(
        'max_step', help="max steps per simulation", type=int, default=-1)
    parser.add_argument('use_gui', help="use gui or not",
